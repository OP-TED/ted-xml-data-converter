...
<!-- in the R2.0.8 schema the form element always has exactly one child element -->
<xsl:variable name="ted-form-fd-element" select="$ted-form-main-element/*"/>

<!-- Variables for child elements of the child element of the first form element, to cater for variations between the schemas -->
<xsl:variable name="ted-form-authority-element" select="$ted-form-fd-element/(*:AUTHORITY_CONCESSION|*:AUTHORITY_ENTITY_DESIGN_CONTEST|*:AUTHORITY_ENTITY_NOTICE_BUYER_PROFILE|*:AUTHORITY_ENTITY_SIMPLIFIED_CONTRACT_NOTICE|*:AUTHORITY_PERIODIC_INDICATIVE|*:AUTHORITY_PRIOR_INFORMATION|*:AUTHORITY_PRIOR_INFORMATION_DEFENCE)"/>


<xsl:variable name="ted-form-award-element" select="$ted-form-fd-element/(*:AWARD_CONTRACT_CONTRACT_AWARD_UTILITIES|*:AWARD_OF_CONTRACT|*:AWARD_OF_CONTRACT_DEFENCE)"/>

<xsl:variable name="ted-form-complementary-element" select="$ted-form-fd-element/(*:COMPLEMENTARY_INFORMATION_CONCESSION|*:COMPLEMENTARY_INFORMATION_CONTRACT_AWARD|*:COMPLEMENTARY_INFORMATION_CONTRACT_AWARD_UTILITIES|*:COMPLEMENTARY_INFORMATION_CONTRACT_CONCESSIONAIRE|*:COMPLEMENTARY_INFORMATION_CONTRACT_NOTICE|*:COMPLEMENTARY_INFORMATION_CONTRACT_NOTICE_DEFENCE|*:COMPLEMENTARY_INFORMATION_CONTRACT_UTILITIES|*:COMPLEMENTARY_INFORMATION_DESIGN_CONTEST|*:COMPLEMENTARY_INFORMATION_NOTICE_BUYER_PROFILE|*:COMPLEMENTARY_INFORMATION_PERIODIC_INDICATIVE|*:COMPLEMENTARY_INFORMATION_QUALIFICATION_SYSTEM|*:COMPLEMENTARY_INFORMATION_RESULT_DESIGN_CONTEST|*:COMPLEMENTARY_INFORMATION_SIMPLIFIED_CONTRACT|*:COMPLEMENTARY_INFORMATION_VEAT|*:OTH_INFO_PRIOR_INFORMATION)"/>

<xsl:variable name="ted-form-contracting-authority-element" select="$ted-form-fd-element/(*:CONTRACTING_AUTHORITY_INFO|*:CONTRACTING_AUTHORITY_INFORMATION|*:CONTRACTING_AUTHORITY_INFORMATION_CONTRACT_AWARD|*:CONTRACTING_AUTHORITY_INFORMATION_CONTRACT_AWARD_DEFENCE|*:CONTRACTING_AUTHORITY_INFORMATION_CONTRACT_SUB_DEFENCE|*:CONTRACTING_AUTHORITY_INFORMATION_DEFENCE|*:CONTRACTING_AUTHORITY_VEAT)"/>

<xsl:variable name="ted-form-contracting-entity-element" select="$ted-form-fd-element/(*:CONTRACTING_ENTITY_CONTRACT_AWARD_UTILITIES|*:CONTRACTING_ENTITY_QUALIFICATION_SYSTEM|*:CONTRACTING_ENTITY_RESULT_DESIGN_CONTEST)"/>

<xsl:variable name="ted-form-lefti-element" select="$ted-form-fd-element/(*:LEFTI_CONCESSION|*:LEFTI_CONTRACT|*:LEFTI_CONTRACT_DEFENCE|*:LEFTI_CONTRACT_NOTICE|*:LEFTI_CONTRACT_NOTICE_UTILITIES|*:LEFTI_CONTRACT_SUB_DEFENCE|*:LEFTI_DESIGN_CONTEST|*:LEFTI_PRIOR_INFORMATION|*:LEFTI_QUALIFICATION_SYSTEM)"/>

<xsl:variable name="ted-form-object-element" select="$ted-form-fd-element/(*:OBJECT_CONCESSION|*:OBJECT_CONTRACT_AWARD_UTILITIES|*:OBJECT_CONTRACT_INFORMATION|*:OBJECT_CONTRACT_INFORMATION_CONTRACT_AWARD_NOTICE|*:OBJECT_CONTRACT_INFORMATION_CONTRACT_AWARD_NOTICE_DEFENCE|*:OBJECT_CONTRACT_INFORMATION_CONTRACT_UTILITIES|*:OBJECT_CONTRACT_INFORMATION_DEFENCE|*:OBJECT_CONTRACT_NOTICE|*:OBJECT_CONTRACT_PERIODIC_INDICATIVE|*:OBJECT_CONTRACT_SUB_DEFENCE|*:OBJECT_DESIGN_CONTEST|*:OBJECT_NOTICE_BUYER_PROFILE|*:OBJECT_QUALIFICATION_SYSTEM|*:OBJECT_RESULT_DESIGN_CONTEST|*:OBJECT_SIMPLIFIED_CONTRACT_NOTICE|*:OBJECT_SUPPLIES_SERVICES_PRIOR_INFORMATION|*:OBJECT_VEAT|*:OBJECT_WORKS_PRIOR_INFORMATION|*:OBJECT_WORKS_SUPPLIES_SERVICES_PRIOR_INFORMATION)"/>

<xsl:variable name="ted-form-procedure-element" select="$ted-form-fd-element/(*:PROCEDURES_CONCESSION|*:PROCEDURES_CONTRACT_AWARD_UTILITIES|*:PROCEDURES_CONTRACT_NOTICE|*:PROCEDURES_DESIGN_CONTEST|*:PROCEDURES_QUALIFICATION_SYSTEM|*:PROCEDURES_RESULT_DESIGN_CONTEST|*:PROCEDURES_SIMPLIFIED_CONTRACT_NOTICE|*:PROCEDURE_ADMINISTRATIVE_INFORMATION_PERIODIC|*:PROCEDURE_DEFINITION_CONTRACT_AWARD_NOTICE|*:PROCEDURE_DEFINITION_CONTRACT_AWARD_NOTICE_DEFENCE|*:PROCEDURE_DEFINITION_CONTRACT_NOTICE|*:PROCEDURE_DEFINITION_CONTRACT_NOTICE_DEFENCE|*:PROCEDURE_DEFINITION_CONTRACT_NOTICE_UTILITIES|*:PROCEDURE_DEFINITION_CONTRACT_SUB_DEFENCE|*:PROCEDURE_DEFINITION_VEAT)"/>
...
<xsl:variable name="eforms-notice-subtype">
	<xsl:value-of select="opfun:get-eforms-notice-subtype($ted-form-element-name, $ted-form-name, $directive, $document-code)"/>
</xsl:variable>
						...
						<xsl:when test="$lot-no ne ''">
							<xsl:choose>
								...
								<xsl:when test="$lot-no-is-convertible">
									<xsl:value-of select="fn:concat('LOT-', functx:pad-integer-to-length(*:LOT_NUMBER, 4))"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="fn:concat('LOT-', *:LOT_NUMBER)"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:when>
...
<xsl:function name="opfun:get-eforms-notice-subtype" as="xs:string">
	...
	<xsl:variable name="notice-mapping-file" select="fn:document('notice-type-mapping-r208.xml')"/>
	<!-- get rows from notice-type-mapping.xml with values matching the given parameters -->
	<xsl:variable name="mapping-row" select="$notice-mapping-file/mapping/row[form-element eq $ted-form-element][form-number eq $ted-form-name][(directive eq $directive) or (directive eq 'ANY')][(document-code eq $ted-form-document-code) or (document-code eq 'ANY')]"/>
