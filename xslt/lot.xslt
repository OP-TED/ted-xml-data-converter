...
<xsl:template name="lot-procurement-project">
	...
	<cac:ProcurementProject>
		...
		<xsl:choose>
			<xsl:when test="fn:normalize-space(fn:string(*:TITLE))"><xsl:apply-templates select="*:TITLE"/></xsl:when>
...
<xsl:template match="*:DATE_START">
	<cac:PlannedPeriod>
		<cbc:StartDate><xsl:value-of select="."/><xsl:text>+01:00</xsl:text></cbc:StartDate>
		<xsl:choose>
			<xsl:when test="../*:DATE_END"><cbc:EndDate><xsl:value-of select="../*:DATE_END"/><xsl:text>+01:00</xsl:text></cbc:EndDate></xsl:when>
			<xsl:otherwise>
